...
def create_proxy_auth_extension(proxy_username, proxy_password, scheme='http', plugin_path='proxy_auth_plugin.zip'):
    manifest_json = """
    {
        "version": "1.0.0",
        "manifest_version": 2,
...
plugin_path = create_proxy_auth_extension(BRIGHT_USERNAME, BRIGHT_PASSWORD)
...
if __name__ == "__main__":
    ...
    options.add_extension(plugin_path)  # Adiciona a extensão de proxy
    
    driver = webdriver.Chrome(options=options)
    wait = WebDriverWait(driver, 20)
    driver.get('https://venda-imoveis.caixa.gov.br/sistema/busca-imovel.asp')
    driver.save_screenshot("screenshotagr.png")
    time.sleep(5)
